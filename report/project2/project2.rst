...
"Extended Addressing" refers to the mechanism by which it is possible to
address the entire program memory on the Atmel ATMega2560 chip.
The instructions found on all AVR chips that change the program counter
only use 16 bit addresses into the program memory.
However, ATMega2560 has more program memory than is addressable
by 16 bits. For this reason, it has instructions that operate on an
address formed by concatenating the 16 bit Z register (R31 + R30) and an
additional EIND register (Extended Indirect Register).

This means that the EIND register is an additional part of a program
execution *context* - the resources virtually independent but physically
shared by different processes. Hence, when switching context, it must be
saved on the stack for the old process and restored from the stack of
the new process along with other registers. This was achieved by
inserting instructions into the context switching assembly code. Note
that the address of the EIND register is specified as 0x3C in the
chip's datasheet.

When saving context, R31 is used as a temporary location for EIND before
it is pushed onto the stack. Hence, this is done after original R31 has been
stored::
...
When restoring context, R31 is used as a temporary location for the
stack entry before it is written into EIND::
...
There are additional implications of Extended Addressing.
The RET instruction
used to return from a function call pops the address of the calling
function (the address in program memory to return to) from the stack.
As explained above, this address is larger than 16 bit on ATMega2560,
and hence 3 bytes are popped from the stack as opposed to 2 bytes on
chips with less program memory.

If we want to re-use code to restore a context in order to begin
execution of a task, its stack must be initially constructed just as
it would be generated by the context-saving code. This means that additional
space for the EIND register must be placed on the stack at task creation.

Moreover, when the kernel resumes a task,
it first switches to the task's stack, restores its context from the stack,
and then uses the RET instruction which pops from the stack the address to return
to. In case a task itself calls a kernel function, instructions to
push the return address onto the stack are generated by the C compiler.
But a newly created task must also have the address of the beginning
of its function stored at the bottom of its stack. In addition,
the address of the function that terminates a task is also stored below
the task function address, to be executed when the task function itself
returns.

The question is, what are the 3 bytes that must be stored on the stack
for each function address? In C language, function pointers on a 16-bit-word
machine are 16 bits themselves. C compilers solve this using the "trampoline"
mechanism: pointers to functions in the extended program memory section are
actually 16-bit pointers into lower parts of program memory that contain
...
TOP       0
          ...
          34    R31
          35    0
          36    Task Function Pointer
          37    Task Function Pointer
          38    0
          39    Task_Terminate Function Pointer
BOTTOM    40    Task_Terminate Function Pointer
========= ===== ===============================

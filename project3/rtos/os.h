 ...
 *
 *   Services are used to transfer data safely between tasks.
 *   A service is created using Service_Init.
 *   New values are published on a service using Service_Publish.
 *   Any number of tasks can subscribe to a service using Service_Subscribe
 *   which returns an object representing a subscription.
 *   Values can be request from the subcription using Service_Receive.
 *   If there are any values published on the service and yet unread through
 *   a subscription, Service_Receive will immediately return the last unread value.
 *   Otherwise, the calling task is suspended until a new value is published
 *   on the service.
 *   Service_Publish makes all tasks waiting for new values on the service
...
typedef struct service_subscription ServiceSubscription;
...
// Use the returned object with Service_Receive.
ServiceSubscription *Service_Subscribe( Service *s );

// Receive the last published and unread value on a service subscription.
